%% >>>> Over Growing Correction
function [fields_data] = over_growing_correction(fields_data_in, basic_penetrating_element_shape, epoch_idx, show)
% >>>> Get Parameter
    ...
    for u_field = 1 : num_original_fields
        ...
        if (fields_mutual_data{i_field, u_field}.neighbours == 1)
            if (fields_mutual_data{i_field, u_field}.may_need_process == 1)
                ...
                [intersection_line_tmp, mask_of_intersection] = intersection_of_two_surfaces_010(fields_data, u_field, i_field, show,shift_from_boarder); 
                ...
                warning(sprintf('no need for process of i-field=%d; u-field=%d, becasue already processed', i_field, u_field));    
                end
            end
        end
    end
...
[intersection_line_tmp, full_mask_of_intersection] = intersection_of_two_surfaces_010(fields_data, u_field, i_field, show, 0);
...
switch basic_penetrating_element.shape
    ...
    otherwise
        error('non supported shape of penetrating element')
end
...
warning('why 12, in the above euation')
...
for ii = 1 : 2
    ...
    for jj = 1 : length(order_of_checking_points)
        ...
        if all(already_in_fields(end+[-1:0]) == in_fields)
            ...
            switch basic_penetrating_element.shape
                case {'disk'}
                    error('this case is not handled now; for the boarder of the penetrating objects');
                case {'line'}
                    if isempty(ponits_tmp)
                        ...
                        elseif ii == 2
                            ...
                            error('  this case is not handled now');
                        end
                    end
...
if move_points_from_i_to_u_field_tmp  % move from i into u_field
    ...
    if size(isolated_points, 1) > 5
        warning('  the minimum number of pixels of a newly created field is set here to 5')
